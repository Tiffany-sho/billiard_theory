...
from setting import wall_width, sinai_wall_height , sinai_circle_diameter,initial_position ,initial_velocity,sinai_set
...
def update(frame):
    ...
    intersection = find_intersection_reversion(position,velocity,wall_width, sinai_wall_height ,sinai_circle_diameter)
    ...
    reflected_velocity = find_reflect_direction(position,velocity,wall_width, sinai_wall_height ,sinai_circle_diameter)
    velocity[:2] = reflected_velocity[:2]


ani = FuncAnimation(fig, update, frames=1000, interval=100 ,repeat=False)
