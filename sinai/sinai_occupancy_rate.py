...
from setting import sinai_poincare_map_arc,occupancy_rate_gragh
...
wall_width =5.0
wall_height =5.0
sinai_circle_diameter = 3.0

range_sin = 2.0
range_arc = 2 * (wall_width  + wall_height + np.pi * sinai_circle_diameter / 2)


position_1,velocity_1 = sinai_create_setting(0,wall_width,wall_height,sinai_circle_diameter,range_arc)
...
def create_axis(initial_position ,initial_velocity,W,H,D,bound_num):
...
def create_poincare_occupany_area(W,H,D,bound_num):
...
def Shannon_entropy(W,H,D,bound_num):
    ...
    plt.scatter(np.log2(bound_num) , -shannon_entropy ,c = "green",s = 5)
...
def shannon_entropy_gragh(W,H,D):
    ...
    for i in range(0,100):
        Shannon_entropy(W,H,D,100 + i * 100)
